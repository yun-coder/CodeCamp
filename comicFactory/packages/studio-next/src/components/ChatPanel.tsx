          ...
          if (ev.type === 'token' && typeof ev.text === 'string') {
            ...
            setMessages((prev) => {
              ...
              if (last && last.role === 'assistant' && (last as Record<string, unknown>)._streaming) {
                ...
                copy.push({ role: 'assistant', content: assistantContent, timestamp: new Date().toISOString(), _streaming: true } as unknown as ChatMessage);
              }
              return copy;
            });
            ...
            setMessages((prev) => {
              const copy = [...prev];
              const last = copy[copy.length - 1];
              if (last && (last as Record<string, unknown>)._streaming) {
                const { _streaming: _, ...final } = last as ChatMessage & { _streaming?: boolean };
