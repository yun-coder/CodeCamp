...
const HV_THINKING_ENV = 'HV_AGENT_THINKING';

function resolveThinking(): { type: 'disabled' } | { type: 'enabled'; budget_tokens: number } | null {
  const raw = process.env[HV_THINKING_ENV];
  if (!raw) return null;
  if (raw === 'disabled' || raw === '0' || raw === 'false') return { type: 'disabled' };
  const budget = parseInt(raw, 10);
  if (!isNaN(budget) && budget > 0) return { type: 'enabled', budget_tokens: budget };
  return null;
}
