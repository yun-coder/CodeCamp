...
function Add_Controls(copy_obj, ev, type) {
    ...
    //todo 更新控件列表
    let append_html = '<tr>\n' +
        '<td ondblclick="edit_name(' + "'" + copy_obj.id + "'" + ',this)">' + copy_obj.id + '</td>\n' +
        '<td class="edit">\n' +
...
//todo 修改控件名
function edit_name(id, obj) {
    //console.log(id);
    obj.innerHTML = '<input type="text" value="' + id + '" onblur="change_name(' + "'" + id + "'" + ',this.value,this);"/>';
}

function change_name(oldval,val, obj) {
    console.log(oldval,val,obj);
    let flag = true;
    for (let k = 0; k < All_Controls.length; k++) {
        if (All_Controls[k].control_name == val) {
            flag = false;
        }
    }
    if(flag){
        obj.parentNode.innerHTML = val;
        let str = JSON.parse($("#" + oldval + " input").val());
        str[0].control_name = val;
        $("#" + oldval + " input").val(JSON.stringify(str));
        $("#" + oldval + " input").attr('name',val);
        $("#"+oldval).attr('id',val);
        for (let k = 0; k < All_Controls.length; k++) {
            if (All_Controls[k].control_name == oldval) {
                All_Controls[k].control_name = val;
                All_Controls[k].control_shape.control_name = val;
            }
        }
    }else{
        toastr.warning('控件名称不能重复');
